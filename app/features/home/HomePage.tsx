...
export function Home() {
  ...
  return (
    <div className="p-8 space-y-10">
      ...
      >
        <img
          src={featuredMod?.imageUrl ?? "/mod-placeholder.svg"}
        ...
        <div className="relative h-full flex flex-col justify-end p-10 max-w-3xl">
          <div className="inline-block px-4 py-1.5 bg-primary/90 backdrop-blur-sm text-primary-foreground text-sm font-semibold rounded-full mb-5 w-fit shadow-lg">
          ...
          <p className="text-foreground/70 mb-8 text-lg leading-relaxed max-w-2xl">
              ...
              <div className="bg-card border border-border rounded-xl overflow-hidden hover:border-primary/30 transition-all group">
                <div className="relative aspect-video overflow-hidden">
                  ...
                  <button
                    ...
                    className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-14 h-14 bg-primary rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all scale-90 group-hover:scale-100"
                  >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          {(loading ? [] : recommendedMods).map((mod, index) => (
            <motion.div
              ...
              initial={{ opacity: 0, y: 20 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.4, delay: Math.min(0.1 + index * 0.07, 0.4) }}
            >
              <ModCard
                id={mod.id}
                title={mod.name}
