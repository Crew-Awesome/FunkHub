...
import { AlertCircle, ArrowLeft, Cpu, Download, Loader2, Search } from "lucide-react";
...
export function AddEngineDialog({ open, onOpenChange }: AddEngineDialogProps) {
  ...
  const filteredEngines = useMemo(() => {
    ...
    if (!normalizedQuery) return enginesCatalog;
  ...
  const handleEngineSelect = (engine: (typeof enginesCatalog)[number]) => {
    setSelectionWarning(null);
    if (engine.slug === "ale-psych" || engine.slug === "psych-online") {
      const proceed = window.confirm(`This is ${engine.name}, not Psych Engine. Continue?`);
      if (!proceed) {
        setSelectionWarning("Selection cancelled.");
        return;
      }
    }
    ...
    setTimeout(() => {
      ...
      if (relOpts.length > 1) setCurrentStep("release");
      else if (srcOpts.length > 1) setCurrentStep("source");
      else if (pkgOpts.length > 1) setCurrentStep("package");
      else setCurrentStep("install");
    }, 0);
  };
            ...
            {!selectedEngine ? (
              ...
              <div className="space-y-5">
                      ...
                      {releaseOptions.map((option) => (
                        <button
                          ...
                          onClick={() => {
                            ...
                            setSelectedSourceKey(null);
                            setSelectedPackageKey(null);
                            if (hasSourceStep) setCurrentStep("source");
                            else setCurrentStep("package");
                          }}
                ...
                {selectedPackage ? (
                  <button
                    type="button"
                    onClick={() => installSelectedEngine(selectedEngine.slug, selectedPackage.downloadUrl, selectedPackage.version)}
